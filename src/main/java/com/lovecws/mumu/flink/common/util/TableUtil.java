    ...
    public static Map<String, Object> getTableInfo(Object object) {
        ...
        if (object instanceof Map) {
            ...
            if (tablePartition != null && tablePartition.partition()) {
                ...
                partitionMap.put("interval", tablePartition.interval());
                partitionMap.put("count", tablePartition.count());
            }
            //索引字段
            ...
            Field[] tableFields = FieldReverseUtils.getFieldsWithAnnotation(object.getClass(), TableField.class);
            if (tableFields.length == 0) {
                Field[] allFields = FieldReverseUtils.getAllFields(object.getClass());
                for (Field field : allFields) {
                    ...
                    columnMap.put("type", field.getType().getTypeName().toLowerCase());
                    columnMap.put("comment", "");
                    //分区字段
        ...
        try {
            if (object instanceof Map) {
                ...
                if (tableFields == null || tableFields.length == 0) {
                    Field[] fields = FieldReverseUtils.getAllFields(object.getClass());
                    columns.forEach(columnMap -> {
                        try {
                            for (Field field : fields) {
                                field.setAccessible(true);
                                if (field.getName().equalsIgnoreCase(columnMap.get("name"))) {
                                    columnValueMap.put(field.getName(), field.get(object));
                                }
                            }
